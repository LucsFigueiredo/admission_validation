...
public class BensValidator
{
    private readonly FileStorageService _storageService;
    private readonly OcrService _ocrService;
    private readonly TextHelper _helperService;

    public BensValidator(OcrService ocrService, FileStorageService storageService, TextHelper helperService)
    {
    ...
    public DocumentValidationDetail ValidateBens(IFormFile file, string candidateName)
    {
        ...
        if (extension != ".png" && extension != ".jpg" && extension != ".jpeg")
        {
            ...
            {       
                ...
                Message = "Declaração de Bens deve ser enviada como imagem"
            };
        }
        ...
        try
        {
            ...
            return new DocumentValidationDetail
            {
                ...
                Message = score >= 60
                    ? $"Declaração de Bens válida e {nameResult.Message}"
                    : $"Declaração de Bens pode estar incorreta e {nameResult.Message}"
            };
        }
        finally
        {
    ...
    public int CalculateScore(string text)
    {
        ...
        if (normalized.Contains("BENS"))
            score += 20;

        if (normalized.Contains("DECLARAÇÃODEAJUSTEANUAL"))
            score += 20;
