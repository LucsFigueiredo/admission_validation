...
public class DiplomaValidator
{
    ...
    public DiplomaValidator(OcrService ocrService, FileStorageService storageService, TextHelper helperService)
    {
    ...
    public DocumentValidationDetail ValidateDiploma(IFormFile file, string candidateName)
    {
        ...
        if (extension != ".png" && extension != ".jpg" && extension != ".jpeg")
        {
            ...
            {       
                ...
                Message = "Diploma ou Certificado Escolar deve ser enviado como imagem"
            };
        }
        ...
        try
        {
            ...
            return new DocumentValidationDetail
            {
                ...
                Message = score >= 60
                    ? $"Diploma ou Certificado Escolar válido e {nameResult.Message}"
                    : $"Diploma ou Certificado Escolar pode estar incorreto e {nameResult.Message}"
            };
        }
        finally
        {
    ...
    public int CalculateScore(string text)
    {
        ...
        if (normalized.Contains("ESCOLAR"))
            score += 20;

        if (normalized.Contains("DIPLOMA") || normalized.Contains("CERTIFICADO"))
            score += 20;
